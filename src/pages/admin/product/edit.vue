...
import {onValue, ref as refDatabase, push, child, update} from "firebase/database";
...
export default {
  ...
  data() {
    return {
      formValue: {
        name: '',
        price: '',
        description: 'sản phẩm này rất hay',
  ...
  created() {
    ...
    const productRef = refDatabase(database, `/products/${this.$route.params.id}`);

    onValue(productRef, snapshot => {
      this.formValue = snapshot.val();
    })
  },
  methods: {
    ...
    submit() {
      ...
      update(refDatabase(database, `/products/${this.$route.params.id}`), {
        ...this.formValue
      })
  ...
  <div>
    <div>
      ...
      >
        ...
        <div>
          <n-button @click="submit">Sửa</n-button>
        </div>
      </n-form>
    </div>
  </div>
